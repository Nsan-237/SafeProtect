...
export const ServicesScreen = ({ navigation }: any) => {
  ...
  const filteredServices = servicesData.filter((s) => {
    ...
    if (selectedCategory === 'Medical') matchesCategory = s.category === 'Medical';
    else if (selectedCategory === 'Legal') matchesCategory = s.category === 'Legal' || s.category === 'Law Enforcement';
    else if (selectedCategory === 'Psychosocial') matchesCategory = s.category === 'Social Services';
    else if (selectedCategory === 'Shelter') matchesCategory = s.category === 'Shelter';
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8F9FE' }}>
      ...
      <View style={{
        ...
        <View style={{
          ...
          <TextInput
            ...
            style={{ flex: 1, paddingVertical: 12, marginLeft: 10, fontSize: 14, color: '#1E1E2D' }}
            value={searchQuery}
      ...
      <View style={{ backgroundColor: '#FFFFFF', paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: '#F0F0F5' }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
        {filteredServices.length === 0 ? (
          <View style={{ alignItems: 'center', marginTop: 48 }}>
            ...
            <Text style={{ color: '#1E1E2D', fontSize: 15, fontWeight: '700', marginTop: 12 }}>No Services Found</Text>
            <Text style={{ color: '#75759E', fontSize: 13, marginTop: 4, textAlign: 'center' }}>
          ...
          filteredServices.map((service) => (
            ...
            >
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: '#75759E', marginRight: 6 }}>
                  {service.distance}
                </Text>
            ...
            >
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                <View style={{
                  ...
                  <Ionicons name={selectedService.icon} size={24} color={selectedService.iconColor} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 18, fontWeight: '800', color: '#1E1E2D' }}>{selectedService.name}</Text>
                  <Text style={{ fontSize: 12, color: '#75759E', fontWeight: '600' }}>{selectedService.category} • {selectedService.distance}</Text>
                </View>
              </View>

              <View style={{ backgroundColor: '#F8F9FE', borderRadius: 14, padding: 16, marginBottom: 20 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
                  <Ionicons name="location-outline" size={18} color="#5B3FD3" style={{ marginRight: 10 }} />
                  <Text style={{ fontSize: 13, color: '#1E1E2D', fontWeight: '600', flex: 1 }}>{selectedService.address}</Text>
                </View>

                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
                  <Ionicons name="time-outline" size={18} color="#5B3FD3" style={{ marginRight: 10 }} />
                  <Text style={{ fontSize: 13, color: '#1E1E2D', fontWeight: '600', flex: 1 }}>{selectedService.hours}</Text>
                </View>

                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <Ionicons name="call-outline" size={18} color="#5B3FD3" style={{ marginRight: 10 }} />
                  <Text style={{ fontSize: 13, color: '#1E1E2D', fontWeight: '600', flex: 1 }}>{selectedService.phone}</Text>
                </View>
              </View>
              ...
              <View style={{ flexDirection: 'row', gap: 12 }}>
                ...
                >
                  ...
                  <Text style={{ color: '#FFFFFF', fontWeight: '800', fontSize: 14 }}>Call Center</Text>
                </TouchableOpacity>
